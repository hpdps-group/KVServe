...
def update_latency_in_json(json_file_path, bandwidth_gbps, original_size_mb, compression_overhead_sec):
    ...
    for record in data:
        if 'cr' in record:
            cr = record['cr']
            new_latency = round(compression_overhead_sec * 1000, 4)
...
# llama
# hadamard ans:0.0721
# hadamard bitcomp:0.0708
# none ans:0.0444
# none bitcomp:0.0431

# qwen
# hadamard ans:0.0339
# hadamard bitcomp:0.033
# none ans:0.0224
# none bitcomp:0.0215

def main():
    parser = argparse.ArgumentParser(description='更新JSON文件中的latency字段')
    parser.add_argument('--json_file', type=str, default='none_merged_gsm8k_llama_ans.json',
    ...
    parser.add_argument('--compression_overhead', type=float, default=0.0444,
