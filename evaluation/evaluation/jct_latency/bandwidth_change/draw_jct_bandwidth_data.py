...
def load_data_from_csv(file_path):
    ...
    # 构建数据字典: data[model][method] = {bw: jct_val, ...}
...
# CSV 文件路径
CSV_PATH = "/root/workspace/KVServe/evaluation/evaluation/jct_latency/bandwidth_change/jct_bandwidth.csv"

# 加载数据
MODELS, BANDWIDTHS, METHODS, DATA_DICT = load_data_from_csv(CSV_PATH)
...
CUSTOM_Y_TICKS_LEFT = [1.4, 1.6, 1.8, 2.0, 11.0]

# 右图 (Qwen) JCT (s)
# 假设范围类似，如果有差异可以在此调整
CUSTOM_Y_TICKS_RIGHT = [4.0, 5.0, 11.0, 50.0]
...
def plot_single_ax(ax, model_name, y_ticks, speedup_idx=0, speedup_x_offset=0.05):
    # 准备 X 轴坐标 (均匀分布)
    ...
    if target_default in y_data_trans and target_kvserve in y_data_trans:
        ...
        if 0 <= idx < len(x_indices):
            ...
            text_x = idx + speedup_x_offset
            
            # 文字
            ax.text(text_x + 0.06, mid_y + 0.05, f"{speedup:.1f}x", 
                    ha='center', va='center', 
            ...
            # 箭头 (向上指 Default)
            ax.annotate('', xy=(text_x, y_def_trans - 0.1), xytext=(text_x, mid_y + 0.2),
                        arrowprops=dict(arrowstyle='->', color=COLOR_MAP[target_kvserve], lw=2.0),
                        zorder=20)
            
            # 箭头 (向下指 KVServe)
            ax.annotate('', xy=(text_x, y_kvs_trans + 0.1), xytext=(text_x, mid_y - 0.1),
                        arrowprops=dict(arrowstyle='->', color=COLOR_MAP[target_kvserve], lw=2.0),
                        zorder=20)
    ...
    # Default xlim is roughly (-0.2, 4.2) for 5 ticks.
    # We want left margin larger -> start at -0.6
    # We want tick spacing smaller -> fit more units in same width -> increase total range -> end at 4.6
    ax.set_xlim(-0.3, len(BANDWIDTHS) - 0.7)

    # ax.set_xlabel("Bandwidth (Gbps)", fontweight='bold', fontsize=16)
    
    # 设置标题
    # ax.set_title(model_name, fontweight='bold', fontsize=16, pad=10)
    # 将标题放在图内部或者上方，这里简单放在上方
    ax.text(0.5, 1, model_name, transform=ax.transAxes, 
...
def main():
    ...
    # 左图: Llama
    plot_single_ax(ax1, "Llama-3.1-8B-Instruct", CUSTOM_Y_TICKS_LEFT, speedup_idx=SPEEDUP_IDX_LEFT, speedup_x_offset=0.05)
    
    # 右图: Qwen
    # 不需要显示Y轴label
    plot_single_ax(ax2, "Qwen2.5-32B-Instruct", CUSTOM_Y_TICKS_RIGHT, speedup_idx=SPEEDUP_IDX_RIGHT, speedup_x_offset=0.05)
    ax2.set_ylabel("")
