...
def draw_chart():
    # 画布大小
    # 注意：之前是 20, 5 对应 4 个图。现在是 2 个图，保持宽度 20 可能太宽，但用户说“画布大小不用变”。
    # 如果用户是指“相对于 draw_dataset_data.py 原本的大小(20,5)不用变”，那就保持 (20, 5)。
    # 这里保持 (20, 5) 以符合 "不用变" 的字面要求，但两个图会比较宽。
    ...
    for i, ax in enumerate(axes):
        ...
        # 调整两侧留白：由于 group_width=0.9，左右边缘大约在 +/-0.45 处
        # 设置为 -0.5 和 num_groups - 0.5 可以让柱子离纵轴更近（仅留 0.05 的空隙）
        ax.set_xlim(-0.57, num_groups - 0.43)
    ...
    # 保存
    output_filename = "jct_latency_dataset_bar_chart.pdf"
    plt.tight_layout()
    # 调整布局以适应顶部图例
    plt.subplots_adjust(top=0.78, bottom=0.15, wspace=0.1)
