...
class MooncakeKVConnectorStats(KVConnectorStats):
    ...
    def __post_init__(self):
        ...
        if not self.data:
            ...
            # Keep snapshots produced by older workers readable while adding
    ...
    _phase_keys = (
        "session_duration",
        "compression_encode_duration",
        "compressed_transfer_duration",
        "compression_decode_duration",
        "cleanup_duration",
    )
    ...
    def record_phase(self, phase: str, duration_s: float) -> None:
        """Record one optional connector phase duration in seconds."""
        if phase not in self._phase_keys:
            raise ValueError(f"Unknown Mooncake timing phase: {phase}")
        if duration_s < 0:
            raise ValueError("Mooncake timing durations must be non-negative")
        with self._lock:
            self.data[phase].append(duration_s)
    ...
    def is_empty(self) -> bool:
        return (
            ...
            and not any(self.data[key] for key in self._phase_keys)
        )

    def aggregate(self, other: KVConnectorStats) -> KVConnectorStats:
        if not other.is_empty():
            for k, v in other.data.items():
                accumulator = self.data.setdefault(k, [])
    ...
    def reduce(self) -> dict[str, int | float]:
        ...
        if self.num_successful_transfers == 0:
            for key in self._phase_keys:
                ...
                if len(values):
                    result[f"Avg {key} (ms)"] = round(values.mean() * 1e3, 3)
            return result
